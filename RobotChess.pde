...
void draw() {  
  ...
  case 2:
    ...
    resign.display();
    if (game_gg == true) {
      log_string += moveString;
      println(log_string);
      String[] conversion_trick = new String[1];
      conversion_trick[0] = log_string;
      saveStrings("nouns.txt", conversion_trick);
      game_state = 3;
    }
...
int line_cnt = 1;
boolean first_move_made = true;
String log_string = "1.";
void log_move(String new_move) {
  
  if(first_move_made){
    log_string += new_move.substring(7, 11);
    log_string += ' ';
    first_move_made = false;
  }else{
    log_string += new_move.substring(7, 11);
    line_cnt++;
    log_string += " " + line_cnt + "." + new_move.substring(12, 16) + " ";
  }
  print(log_string);
}
void drawPieces() {
...
void lossCard() {
  ...
  if (player_time <= 0) {
    ...
    result = "You lost by resignation";
  }
